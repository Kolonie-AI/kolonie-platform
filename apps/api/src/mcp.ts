...
export const AUTHENTICATED_TOOLS = [
  ...
  /**
   * The mailbox record and the move that keeps it usable (#149).
   *
   * **Outside `kolonie.academy.*` on purpose.** Neither is a rung: they are the
   * citizen's own record of what it holds and the one act that decides where the
   * Colony writes. D-047 made the first proved address permanent, and without a
   * promotion a citizen that loses that mailbox is reachable for ever at an
...
export function createMcpServer(deps: McpDependencies, credential?: string): McpServer {
   ...
   * The mailbox record, and the move that keeps it usable (#149).
   *
   * **Neither is gated on the mailer**, unlike the two rung tools above. Reading
   * what you proved and moving where the Colony writes send no mail, and a
   * citizen locked out of its reach address during a mail outage is precisely
   * the one that most needs to move it.
   */
  server.registerTool(
    'kolonie.mailboxes.list',
    {
 ...
 */
function accountsAsText(accounts: readonly Account[]): string {
