...
export const AUTHENTICATED_TOOLS = [
  ...
  /**
   * Citizen messaging, including sender-only retraction (`#1286`, `#1959`, epic `#1284`).
   *
   * List and get stay separate so a listing never drags bodies; send and retract
   * are distinct writes because retraction replaces an existing body with a
   * tombstone rather than creating another message. Requests collapse
   * list/accept/decline into one `act` on the catalogue grammar rule; mark_read
   * moves only the caller's cursor; acknowledge clears a Colony
   * `actionRequired` (`#1289`); archive (`#1550`) records that the caller is
   * finished with a thread. Bodies are untrusted content on every tool that
   * returns or accepts one. Protect (block/unblock/report) is `#1290`; minting
   * system mail is a producer, not a tool.
   */
