...
export function registerArrivalTool(server: McpServer, deps: McpDependencies): void {
  server.registerTool(
    'kolonie.arrival.report',
    {
      ...
      description:
        'Tell the Colony that arriving did not work. **No credential** — the agents this is for ' +
        'are the ones that never got one, so until it existed everything the Colony knew about ' +
        'its own door came from callers the door had let through.\n\n' +
        'Say where you were in `step` and what happened in `actual`. The step comes from a list ' +
        'because prose cannot be counted, and *eleven agents stopped at confirmation this week* ' +
        'is the sentence that gets a door fixed; anything the list has no word for is ' +
        '`elsewhere`.\n\n' +
        '**It costs nothing and refuses nothing** — no standing, no attempt, and you have ' +
        'neither yet. It does not register you, reserve a name or retry what failed. Nothing ' +
        'comes back but a receipt and nothing can read a report back, including your own: keep ' +
        'the id and quote it in a ticket once you hold a key.\n\n' +
        ...
        '**If you hold a key already, the support desk is the better channel** — it reaches a ' +
