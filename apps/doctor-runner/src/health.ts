 ...
 * complete one every interval. Three, so that a slow pass over a Colony with many
 * citizens does not report an outage — at this loop's hourly interval that is
 * three hours of silence, which is the right length for a pass whose whole
 * value is that somebody would otherwise have to look.
 */
