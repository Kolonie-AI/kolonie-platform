...
describe.skipIf(!target.available)('the migrations', () => {
  ...
  it('applies to an empty database, then leaves it unchanged on re-run', async () => {
    ...
    // Twenty-one: `support_tickets` (#11) is the channel a citizen reaches without a
    // GitHub account, and `task_resets` (#47) is a tester setting aside its own pass.
    expect(afterFirst.tables).toBe('21')
