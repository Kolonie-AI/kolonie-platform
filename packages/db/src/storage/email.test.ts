...
describe.skipIf(!target.available)('email round trip', () => {
  ...
  describe('proving you can read', () => {
    ...
    /**
     * The rejection case #121 is about. A mismatched mail leaves `inbound_at`
     * null — the sender comparison lives in the `UPDATE`'s `WHERE` — so before
     * this the agent was told nothing had been sent, which is false and rules
     * out the one explanation that fits. An agent given that answer re-sends
     * from the same wrong address.
     */
    it('separates "nothing arrived" from "something arrived from the wrong address"', async () => {
      const challenge = await mint(agentId, 'citizen@example.org')
      ...
      expect(await recordInboundMail(db, challenge.token, 'someone-else@example.org')).toEqual({
        outcome: 'sender_mismatch',
      })

      expect(await redeemEmailCode(db, agentId, 'AAAAAAAAAAAA')).toEqual({
        outcome: 'sender_mismatched',
        arrivedFrom: 'someone-else@example.org',
        claimed: 'citizen@example.org',
      })
    })

    /** The reverse rejection: silence must not be reported as a mismatch. */
    it('still reports silence as silence after a matching mail is recorded', async () => {
      const challenge = await mint(agentId, 'citizen@example.org')
      await recordInboundMail(db, challenge.token, 'citizen@example.org')

      // A mail did arrive and matched, so the mismatch note must never be
      // reached — there is a code waiting, and `wrong_code` is the true answer.
      expect(await redeemEmailCode(db, agentId, 'AAAAAAAAAAAA')).toEqual({ outcome: 'wrong_code' })
    })

    /**
     * A stray mail *after* a successful arrival is not the agent's problem: it
     * has a code waiting, and overwriting the row then would replace a live
     * proof with a diagnostic about somebody else's mail.
     */
    it('does not record a mismatch once the challenge has already been satisfied', async () => {
      const challenge = await mint(agentId, 'citizen@example.org')
      await recordInboundMail(db, challenge.token, 'citizen@example.org')

      expect(await recordInboundMail(db, challenge.token, 'stranger@example.org')).toEqual({
        outcome: 'sender_mismatch',
      })
      expect((await latestEmailChallenge(db, agentId))?.mismatchedFrom).toBeNull()
    })

    /** The most recent mismatch is the useful one; this is not a log. */
    it('keeps the address of the most recent mismatched mail', async () => {
      const challenge = await mint(agentId, 'citizen@example.org')
      await recordInboundMail(db, challenge.token, 'first@example.org')
      await recordInboundMail(db, challenge.token, 'second@example.org')

      expect((await latestEmailChallenge(db, agentId))?.mismatchedFrom).toBe('second@example.org')
    })

    /**
     * A mismatch is a diagnostic and never a verdict: the round trip still
     * completes when the right mail finally arrives.
     */
    it('lets the agent pass after a mismatch, once it sends from the claimed address', async () => {
      const challenge = await mint(agentId, 'citizen@example.org')
      await recordInboundMail(db, challenge.token, 'wrong@example.org')
